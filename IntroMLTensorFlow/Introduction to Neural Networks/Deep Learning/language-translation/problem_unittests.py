...
def test_text_to_ids(text_to_ids):
    ...
    source_vocab_to_int, source_int_to_vocab = helper.create_lookup_tables(
        test_source_text
    )
    target_vocab_to_int, target_int_to_vocab = helper.create_lookup_tables(
        test_target_text
    )

    test_source_id_seq, test_target_id_seq = text_to_ids(
        test_source_text, test_target_text, source_vocab_to_int, target_vocab_to_int
    )

    assert len(test_source_id_seq) == len(
        test_source_text.split("\n")
    ), "source_id_text has wrong length, it should be {}.".format(
        len(test_source_text.split("\n"))
    )
    assert len(test_target_id_seq) == len(
        test_target_text.split("\n")
    ), "target_id_text has wrong length, it should be {}.".format(
        len(test_target_text.split("\n"))
    )

    target_not_iter = [
        type(x) for x in test_source_id_seq if not isinstance(x, collections.Iterable)
    ]
    assert (
        not target_not_iter
    ), "Element in source_id_text is not iteratable.  Found type {}".format(
        target_not_iter[0]
    )
    target_not_iter = [
        type(x) for x in test_target_id_seq if not isinstance(x, collections.Iterable)
    ]
    assert (
        not target_not_iter
    ), "Element in target_id_text is not iteratable.  Found type {}".format(
        target_not_iter[0]
    )

    source_changed_length = [
        (words, word_ids)
    ...
    assert (
        ...
        len(source_changed_length[0][0].split()),
        len(source_changed_length[0][1]),
        source_changed_length[0][1],
    )

    target_missing_end = [
        word_ids
        for word_ids in test_target_id_seq
        if word_ids[-1] != target_vocab_to_int["<EOS>"]
    ]
    assert not target_missing_end, "Missing <EOS> id at the end of {}".format(
        target_missing_end[0]
    )

    target_bad_size = [
        (words.split(), word_ids)
    ...
    source_bad_id = [
        ...
        for word, word_id in zip(
            [
                word
                for sentence in test_source_text.split("\n")
    ...
    assert (
        not source_bad_id
    ), "Source word incorrectly converted from {} to id {}.".format(
        source_bad_id[0][0], source_bad_id[0][1]
    )
    ...
    target_bad_id = [
        ...
        for word, word_id in zip(
            [
                word
                for sentence in test_target_text.split("\n")
                for word in sentence.split()
            ],
    ...
    assert (
        not target_bad_id
    ), "Target word incorrectly converted from {} to id {}.".format(
        target_bad_id[0][0], target_bad_id[0][1]
    )
...
def test_model_inputs(model_inputs):
    with tf.Graph().as_default():
        ...
        assert (
            keep_prob.op.type == "Placeholder"
        ), "Keep Probability is not a Placeholder."
        assert (
            target_sequence_length.op.type == "Placeholder"
        ), "Target Sequence Length is not a Placeholder."
        assert (
            max_target_sequence_length.op.type == "Max"
        ), "Max Target Sequence Length is not a Placeholder."
        assert (
            source_sequence_length.op.type == "Placeholder"
        ...
        ), "Source Sequence Length has bad name.  Found name {}".format(
            source_sequence_length.name
        )
        assert (
        ...
        assert tf.assert_rank(
            target_sequence_length, 1, message="Target Sequence Length has wrong rank"
        )
        assert tf.assert_rank(
            max_target_sequence_length,
            0,
        ...
        assert tf.assert_rank(
            source_sequence_length, 1, message="Source Sequence Lengthhas wrong rank"
        )
...
def test_encoding_layer(encoding_layer):
    ...
    with tf.Graph().as_default():
        ...
        enc_output, states = encoding_layer(
            rnn_inputs,
            rnn_size,
            num_layers,
            keep_prob,
        ...
        bad_types = [
            type(state)
            for state in states
            if not isinstance(state, tf.contrib.rnn.LSTMStateTuple)
        ]
        ...
        bad_shapes = [
            ...
            if state_tensor.get_shape().as_list()
            not in [[None, rnn_size], [batch_size, rnn_size]]
        ]
...
def test_decoding_layer(decoding_layer):
    ...
    with tf.Graph().as_default():
        ...
        target_sequence_length_p = tf.placeholder(
            tf.int32, (None,), name="target_sequence_length"
        )
        max_target_sequence_length = tf.reduce_max(
            target_sequence_length_p, name="max_target_len"
        )

        dec_input = tf.placeholder(tf.int32, [batch_size, sequence_length])
        dec_embed_input = tf.placeholder(
            tf.float32, [batch_size, sequence_length, embedding_size]
        )
        ...
        train_decoder_output, infer_logits_output = decoding_layer(
            dec_input,
            encoder_state,
        ...
        assert train_decoder_output.rnn_output.get_shape().as_list() == [
            ...
            None,
            vocab_size,
        ], "Wrong shape returned.  Found {}".format(
        ...
        assert infer_logits_output.sample_id.get_shape().as_list() == [
            ...
            infer_logits_output.sample_id.get_shape()
        )
...
def test_seq2seq_model(seq2seq_model):
    ...
    with tf.Graph().as_default():
        ...
        train_decoder_output, infer_logits_output = seq2seq_model(
            input_data,
            target_data,
            keep_prob,
            batch_size,
...
def test_sentence_to_seq(sentence_to_seq):
    ...
    assert np.array_equal(output, [3, 6, 5, 2, 4]), "Incorrect ouput. Found {}".format(
        output
    )
...
def test_process_encoding_input(process_encoding_input):
    ...
    with tf.Graph().as_default():
        ...
        assert (
            test_dec_input[0][0] == target_vocab_to_int["<GO>"]
            and test_dec_input[1][0] == target_vocab_to_int["<GO>"]
        ), "Missing GO Id."
...
def test_decoding_layer_infer(decoding_layer_infer):
    ...
    with tf.Graph().as_default():
        with tf.variable_scope("decoding") as decoding_scope:
            ...
            infer_logits_output = decoding_layer_infer(
                encoder_state,
                dec_cell,
                dec_embeddings,
                1,
