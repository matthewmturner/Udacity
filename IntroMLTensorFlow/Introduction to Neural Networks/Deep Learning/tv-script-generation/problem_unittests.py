...
def test_get_batches(get_batches):
    with tf.Graph().as_default():
        ...
        # Check type
        assert isinstance(batches, np.ndarray), "Batches is not a Numpy array"
        ...
        for x in range(batches.shape[2]):
            assert np.array_equal(
                batches[0, 0, x], np.array(range(x * 35, x * 35 + batches.shape[3]))
...
def test_tokenize(token_lookup):
    with tf.Graph().as_default():
        ...
        # Check type
        assert isinstance(token_dict, dict), "Returned type is {}.".format(
            type(token_dict)
        )
...
def test_get_inputs(get_inputs):
    with tf.Graph().as_default():
        ...
        # Check type
        assert input_data.op.type == "Placeholder", "Input not a Placeholder."
        assert targets.op.type == "Placeholder", "Targets not a Placeholder."
        assert lr.op.type == "Placeholder", "Learning Rate not a Placeholder."
        ...
        # Check rank
        input_rank = (
            0 if input_data.get_shape() == None else len(input_data.get_shape())
        )
        targets_rank = 0 if targets.get_shape() == None else len(targets.get_shape())
        lr_rank = 0 if lr.get_shape() == None else len(lr.get_shape())

        assert input_rank == 2, "Input has wrong rank.  Rank {} found.".format(
            input_rank
        )
        assert targets_rank == 2, "Targets has wrong rank. Rank {} found.".format(
            targets_rank
        )
        assert lr_rank == 0, "Learning Rate has wrong rank. Rank {} found".format(
            lr_rank
        )
...
def test_get_embed(get_embed):
    with tf.Graph().as_default():
        ...
        # Check shape
        assert embed.shape == embed_shape, "Wrong shape.  Found shape {}".format(
            embed.shape
        )
...
def test_build_rnn(build_rnn):
    with tf.Graph().as_default():
        ...
        test_cell = rnn.MultiRNNCell(
            [rnn.BasicLSTMCell(test_rnn_size) for _ in range(test_rnn_layer_size)]
        )
...
def test_build_nn(build_nn):
    with tf.Graph().as_default():
        ...
        logits, final_state = build_nn(
            test_cell, test_rnn_size, test_input_data, test_vocab_size, test_embed_dim
        )
        ...
        assert (
            final_state.name == "final_state:0"
        ), "Final state doesn't have the correct name. Found the name {}. Are you using build_rnn?".format(
            final_state.name
        )
...
def test_get_tensors(get_tensors):
    ...
    assert input_text == test_input, "Test input is wrong tensor"
    assert initial_state == test_initial_state, "Initial state is wrong tensor"
    assert final_state == test_final_state, "Final state is wrong tensor"
    assert probs == test_probs, "Probabilities is wrong tensor"
...
def test_pick_word(pick_word):
    with tf.Graph().as_default():
        ...
        test_int_to_vocab = {
            word_i: word for word_i, word in enumerate(["this", "is", "a", "test"])
        }
        ...
        assert (
            pred_word in test_int_to_vocab.values()
